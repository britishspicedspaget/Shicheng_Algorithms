...
int main()
{
        ...
        //defining the x range of the graph
        int x1, x2;
        printf("Select the x range for your graphing:");
        scanf("%d %d",&x1,&x2);
        head:
        if((ABS(x2 - x1) + 1) != size)
        {
                printf("\nPlease re-enter your size of the x range(entered %d, defined maximum %d):",ABS(x2 - x1) + 1, size);
                scanf("%d %d",&x1, &x2);
                goto head;
        }
        double x;
